...
const fetchInProgressPlans = async () => {
  ...
  try {
    const res = await axios.post('http://127.0.0.1:8000/generate-user-records/in-progress', {
...
const fetchStartingSoonPlans = async () => {
  ...
  try {
    const res = await axios.post('http://127.0.0.1:8000/generate-user-records/starting-soon', {
      user_id: Number(userId.value),
